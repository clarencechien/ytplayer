...
> **2026-08-13 實測更新（日文部分）**：上表對日文**過於悲觀**，前提已不成立。
> YouTube ASR 換代（`variant=gemini`，有標點與大小寫）後，日文轉寫的**內容正確性高**，
> 錯誤窄而系統性地集中在產品名（`Fold`→`old` 共 6 次），而那正是修稿 pass 的標的且答案就在影片標題裡。
> 詳見 [asr-language-experiment.md](asr-language-experiment.md)。**韓文仍未量測，維持原假設。**
