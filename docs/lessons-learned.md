...
- **v5 回聲對位**：輸出帶 `t`（原文前 12 字），對不上就丟回重譯 —— 讓「這句譯文對到哪句原文」
  可驗證。抓到過真實的整段位移；但**修不了子句邊界漂移**，兩者別混為一談
- **v6 字數預算**：把顯示秒數換算成 `[≤N 字]` 寫進 prompt，讓模型自己壓縮
  （docs/subtitle-readability.md）。**是上限不是目標**，寫「≤N 字」不能寫「N 字」——
  模型會湊字數。實測 CPS>12 -29%、tokens 只 +0.3%、沒被標的句子完全不動
...
- **修法要分「零成本」與「花錢」兩層，零成本的先跑**：📏 壓縮先剝掉英文夾註再重算 CPS，
  達標的根本不呼叫 API（實測一半的超標句免費解決）。同一條原則早就用在 ⏱ 修時間上
- **品質改善要問「舊片怎麼套」**：R1 只作用在之後翻的片，所以配一顆 📏 按鈕；
  而按鈕要出現，`/jobs.json` 就得從 bilingual.json 回填舊片沒有的計數欄位 ——
  少了回填，事後套用路徑等於不存在
...
- **儀表板的數字要能被信任**：`/admin` 的耗時本來用 `updatedAt - startedAt` 算，
  事後補譯會推進 `updatedAt` —— 五天前翻的片按一次按鈕就顯示「7229 分」，
  看起來像失控燒錢，實際只花了 NT$11.68。現在用 assemble 當下記的 `doneAt`。
  同理「更新於」只印時間不印日期，舊片會看起來像剛跑完。
  **一個顯示 bug 會讓人以為是成本事故**，代價是一輪查帳的時間
