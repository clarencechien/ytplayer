...
4. 已接受的殘餘風險（記錄在案）：任何人可探測 `/subs/{id}/info.json` 的 200/404，
   得知「某支影片有沒有被我翻過」。**自用容忍，但這一條原本的敘述低估了實情**
   （2026-09-04 更正）：

   - videoId **不是秘密**。它印在每一個 YouTube 網址上，11 碼的空間雖然大到不能窮舉，
     但「我想知道你有沒有翻過**這一支**」從來不需要窮舉 —— 針對性的探測是一次請求的事。
   - `/subs/*` 同時**繞過 Worker 的 bot 閘門與 zone 上的 UA 型 WAF 規則**。
     那是刻意的：README 教的 `curl …/subs/<id>/bilingual.srt` 與播放器頁面都靠它。
     但也表示這個端點沒有任何速率或來源限制。
   - 洩漏的內容是「這個站上有哪些片被翻過」的成員關係，以及 `info.json` 裡的標題。
     不是字幕內容本身（那要另外抓），但足以推出觀看興趣。

   **為什麼還是維持公開**：另一個選項是每片一個分享 token（`/subs/{id}?t=…`），
   代價是既有的播放器連結全部失效、擴充要改、README 的 curl 流程要重寫，
   而換到的是「隱藏成員關係」——對一個自用站，那個交換不划算。
   真的要做的話，做法是在 `/subs` 路由前加一道 token 檢查，token 由
   `/translate` 完成時產生並寫進 `info.json`，播放器頁面在伺服器端把它嵌進連結。
