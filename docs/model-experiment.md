...
## 追加：G 組 — 3.6-flash 重評、H 組 — lite × 協定（2026-08-16）

完整設計、總表與人工抽樣見 [exp-2026-08-16.md](exp-2026-08-16.md)。摘要：

| 條件 | 未譯 | tokens | NT$／片 | 秒 | 結論 |
|---|---|---|---|---|---|
| **3.5-flash v5（維持預設）** | 0（6 次）| 49.4k | 6.08 | 30 | — |
| 3.6-flash v5 | 0–2 | 66.4k | 6.41 | 136 | **維持禁用**：更慢 4.5 倍、更貴、未譯更多 |
| lite + id 協定 | 1–4 | 122k | 2.69 | 32 | 重試 31–41 次 —— id 紀律的代價 |
| lite + 位置對齊 | 0–1 | 48.7k | **0.89** | 15 | **可用**：大量補翻舊片的選項 |

兩個要記住的教訓：

1. **3.6 的禁用理由變了**：原本是「會譯錯句」（已被 F1 回聲對位擋住，`echoRejects` 與 3.5 同量級），
   現在是「沒有優勢」。**重評的價值有時不是換掉，而是把過期的理由換成當下的理由。**
2. **費率一定要逐模型分開算**：runner 原本寫死 flash 費率，把 lite 高估 5 倍，
   差點誤判 F2 的成本 gate（「只降 37%」實際是「降 85%」）。已修成費率表。
