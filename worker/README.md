...
| `GEMINI_THINKING_BUDGET` | Var（選填） | text 路由 thinking token 上限，**預設 128 = 實質關閉**（0 會被 3.6-flash 拒收，128 兩者通吃且實測 thoughts=0） — 翻譯是機械轉換不需推理，thinking 以輸出價計費（2026-08-13 帳單事故主因：Output 是 Input 的 3–4 倍全是思考）。看片路由不受此限（維持模型預設） |
